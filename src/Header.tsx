...
import { adminLinkColumnistUser, adminUnlinkColumnistUser } from "@/lib/admin.functions";
...
export function ColumnistsManager() {
  ...
  const linkUser = useServerFn(adminLinkColumnistUser);
  const unlinkUser = useServerFn(adminUnlinkColumnistUser);
  ...
  const linkAccess = async () => {
    if (!editing?.id || !linkEmail.trim()) return;
    setLinking(true);
    setLinkMessage("");
    try {
      const res = await linkUser({ data: { columnistId: editing.id, email: linkEmail.trim() } });
      if (res.ok) {
        setLinkMessage("Acesso vinculado com sucesso!");
        setLinkEmail("");
        refresh();
      } else {
        setLinkMessage(res.error ?? "Erro ao vincular.");
      }
    } finally {
      setLinking(false);
    }
  ...
  const unlinkAccess = async () => {
    ...
    if (!confirm("Remover o acesso deste colunista ao painel? Ele não conseguirá mais editar a própria coluna até vincular de novo.")) return;
    setLinking(true);
    setLinkMessage("");
    try {
      const res = await unlinkUser({ data: { columnistId: editing.id } });
      if (res.ok) {
        setEditing({ ...editing, user_id: null });
        refresh();
      } else {
        setLinkMessage(res.error ?? "Erro ao desvincular.");
      }
    } finally {
      setLinking(false);
    }
  ...
  if (editing) {
    return (
      <form onSubmit={submit} className="mx-auto max-w-2xl space-y-4">
        ...
        {editing.id && (
          <div className="rounded-2xl border bg-sky-soft/40 p-4">
            <p className="mb-1 font-display text-xs font-black uppercase tracking-widest text-muted-foreground">
              Acesso ao painel (editar e publicar a própria coluna)
            </p>
            {editing.user_id ? (
              ...
              <div className="mt-2 space-y-2">
                <p className="text-xs text-muted-foreground">
                  O colunista precisa primeiro criar a própria conta em{" "}
                  <span className="font-semibold text-foreground">/auth</span> ("Primeira vez? Criar
                  conta"). Depois, informe o mesmo e-mail aqui para vincular.
                </p>
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="e-mail da conta do colunista"
                    className="h-10 flex-1 rounded-xl border bg-card px-3 text-sm outline-none ring-ring focus:ring-2"
                  />
                  <button
                    type="button"
                    onClick={linkAccess}
                    disabled={linking || !linkEmail.trim()}
                    className="rounded-xl bg-primary px-4 text-sm font-bold text-primary-foreground hover:bg-primary/90 disabled:opacity-60"
                  >
            ...
            {linkMessage && <p className="mt-2 text-sm text-muted-foreground">{linkMessage}</p>}
          </div>
        )}
      </form>
    );
              ...
              <div className="flex items-start gap-3 p-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    ...
                    {c.user_id && <span className="ml-2 text-primary">· login vinculado</span>}
                  </p>
