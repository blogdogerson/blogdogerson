...
export function SponsorsBand({ banners }: { banners: Banner[] }) {
  const items = banners.filter((b) => b.active).slice(0, 8);
  ...
  return (
    <section className="mt-14 sm:mt-20">
      ...
      {items.length === 0 ? (
        <Link
          to="/anuncie"
          className="flex min-h-32 items-center justify-center rounded-3xl border-2 border-dashed border-primary/30 bg-sky-soft text-center transition-colors hover:border-primary"
        >
          <div>
            <p className="font-display text-lg font-black uppercase tracking-wider text-primary">
              Espaço reservado para patrocinadores
            </p>
            <p className="mt-1 text-sm text-muted-foreground">
              Anuncie no Blog do Gerson · gerson@blogdogerson.com.br
            </p>
          </div>
        </Link>
      ) : (
