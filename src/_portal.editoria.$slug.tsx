  ...
  .handler(async ({ data, context }) => {
    ...
    const rows = posts.map((p) => {
      const wpCats: string[] = (p.categories ?? [])
        ...
        .filter(Boolean);
      // Prioriza a categoria mais específica (cidade/editoria) sobre "Geral":
      // muitos posts do WordPress trazem "Geral" como primeira categoria mesmo
      // sendo, na prática, de Gramado/Canela/etc. — só cai em Geral se nenhuma
      // categoria mais específica for encontrada.
      const mapped = wpCats.map((n) => WP_CATEGORY_MAP[n as string]).filter(Boolean) as string[];
      const category = mapped.find((c) => c !== "Geral") ?? mapped[0] ?? "Geral";
      const image =
