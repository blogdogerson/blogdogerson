...
export function ArticlesManager() {
  ...
  const topics = catData?.categories ?? [];
  // Se a notícia em edição tem uma categoria que não existe mais como tópico
  // (ex.: "Geral", removido do painel), mantém ela na lista para não perder
  // a informação nem trocar a categoria sem querer ao salvar.
  const categoryOptions =
    editing && editing.category && !topics.some((t: any) => t.name === editing.category)
      ? [{ id: "current", name: editing.category }, ...topics]
      : topics;
    ...
    return (
      <form onSubmit={submit} className="mx-auto max-w-3xl space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          >
            {categoryOptions.length === 0 && <option value="">Nenhum tópico cadastrado</option>}
  ...
  return (
    <div>
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        />
        <button
          onClick={() => setEditing({ ...EMPTY, category: topics[0]?.name ?? "" })}
